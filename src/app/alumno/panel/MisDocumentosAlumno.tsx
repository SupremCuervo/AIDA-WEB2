...
import { IconoDescargar, IconoDocumento, IconoSubir } from "@/app/alumno/aida-iconos";
...
export default function MisDocumentosAlumno() {
	...
	async function enviarArchivo(tipo: TipoDocumentoClave, file: File) {
		...
		try {
			...
			if (!res.ok) {
				setMensaje({ tipo: "error", texto: data.error ?? "No se pudo subir el archivo" });
				return;
			}
	...
	return (
		<section className="relative overflow-hidden rounded-2xl border border-[#E2E8F0] bg-[#FFFFFF] shadow-xl shadow-[#2563EB]/10">
			<div className="absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r from-[#2563EB] via-[#3B82F6] to-[#7C3AED]" />
			<div className="p-5 sm:p-8">
				<div className="flex items-start gap-3">
					<span className="mt-0.5 flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[#DBEAFE] text-[#2563EB]">
						<IconoDocumento className="h-6 w-6" />
					</span>
					<div>
						<h2 className="text-lg font-semibold text-[#1E293B] sm:text-xl">Mis documentos</h2>
						<p className="mt-2 max-w-3xl text-sm leading-relaxed text-[#64748B]">
							<span className="inline-flex items-center gap-1.5 rounded-md bg-[#EFF6FF] px-2 py-0.5 text-xs font-medium text-[#2563EB] ring-1 ring-[#DBEAFE]">
								Sin archivo
							</span>{" "}
							<span className="text-[#64748B]">·</span>{" "}
							<span className="inline-flex items-center gap-1.5 rounded-md bg-[#F5F3FF] px-2 py-0.5 text-xs font-medium text-[#7C3AED] ring-1 ring-[#EDE9FE]">
								Subido
							</span>{" "}
							<span className="text-[#64748B]">·</span>{" "}
							<span className="inline-flex items-center gap-1.5 rounded-md bg-teal-50 px-2 py-0.5 text-xs font-medium text-teal-800 ring-1 ring-teal-200">
								Aceptado
							</span>{" "}
							<span className="text-[#64748B]">·</span>{" "}
							<span className="inline-flex items-center gap-1.5 rounded-md bg-red-50 px-2 py-0.5 text-xs font-medium text-red-800 ring-1 ring-red-200">
								Rechazado
							</span>
						</p>
					</div>
						...
						<table className="w-full min-w-[min(100%,720px)] border-collapse text-left text-sm">
							<thead>
								<tr className="border-b border-[#E2E8F0] bg-[#FFFFFF]">
									<th className="whitespace-nowrap px-4 py-3.5 text-xs font-semibold uppercase tracking-wide text-[#64748B] sm:px-5">
										Documento
									</th>
									<th className="whitespace-nowrap px-4 py-3.5 text-xs font-semibold uppercase tracking-wide text-[#64748B] sm:px-5">
										Estatus
								...
								{cargandoTabla ? (
									...
									(Object.keys(TIPOS_DOCUMENTO) as TipoDocumentoClave[]).map((tipo) => {
										...
										return (
											<tr
												key={tipo}
												className="transition-colors hover:bg-[#EFF6FF]/50"
											>
												<td className="px-4 py-4 align-middle sm:px-5">
													...
													<span className="mt-0.5 block pl-6 text-xs text-[#94A3B8]">
														<code className="rounded bg-[#F1F5F9] px-1 py-0.5 text-[11px] text-[#64748B]">
															{tipo}
														</code>
													</span>
												</td>
