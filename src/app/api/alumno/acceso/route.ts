...
export async function POST(request: Request) {
	...
	let payloadClave: { grupoTokenId: string; grupo: string; grado: string };
	try {
	...
	try {
		...
		if (await grupoTokenEstaVencido(supabase, payloadClave.grupoTokenId)) {
		...
		const gradoSesion = gradoMostradoParaAlumno(filaPadron.grado_alumno, payloadClave.grado);
